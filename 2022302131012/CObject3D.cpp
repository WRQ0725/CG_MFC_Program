...
void CObject3D::CreateBall(double x0, double y0, double z0, double r)
{
	...
	for (double u = -pi / 2; u < pi / 2 + pi / 32; u = u + pi / 32)
	{
		...
		for (double v = 0; v < 2 * pi - pi / 32; v = v + pi / 32)
		{
			...
			//计算该点的单位法向量
			double d = sqrt((p3d[i][j].x - x0) * (p3d[i][j].x - x0)
				+ (p3d[i][j].y - y0) * (p3d[i][j].y - y0)
				+ (p3d[i][j].z - z0) * (p3d[i][j].z - z0));
			p3d[i][j].xn = (p3d[i][j].x - x0) / d;
			p3d[i][j].yn = (p3d[i][j].y - y0) / d;
			p3d[i][j].zn = (p3d[i][j].z - z0) / d;
			j++;
		}
		i++;
	}
